            ...
            <form action="{{ $isEdit ? route('mahasiswa.update', $mahasiswa->id) : route('mahasiswa.store')}}" method="POST">
              ...
              <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                  ...
                  <div class="mt-4 text-left">
                      ...
                      <input type="text" name="nim" id="nim" value="{{ $mahasiswa->nim ?? '' }}" class="block w-full px-3 py-2 mt-1 border rounded-md shadow-sm focus:outline-none focus:ring focus:ring-blue-500 sm:text-sm">
                  </div>

                  <div class="mt-4 text-left">
                      <label for="name" class="block text-sm font-medium text-gray-700">Nama</label>
                      <input type="text" name="name" id="name" value="{{ $mahasiswa->name ?? '' }}" class="block w-full px-3 py-2 mt-1 border rounded-md shadow-sm focus:outline-none focus:ring focus:ring-blue-500 sm:text-sm">
                  </div>
                  <div class="mt-4 text-left">
                    ...
                    <select name="kelas_id" id="kelas_id" class="border border-gray-400 px-3 py-2 rounded-md w-full">
                      ...
                      <option value="{{ $kls->id }}" {{ (old('kelas_id') ?? ($mahasiswa->kelas_id ?? '')) == $kls->id ? 'selected' : '' }}>{{ $kls->name }}</option>
                      @endforeach
                    </select>
                  </div>
